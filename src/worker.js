...
async function login(request, env, corsHeaders) {
  ...
  if (!user) {
    return new Response(JSON.stringify({ error: "Nieprawidłowy PIN" }), {
      status: 401,
      headers: corsHeaders,
    });
  }
...
async function createLocation(request, env, corsHeaders) {
  ...
  if (!name) {
    return new Response(JSON.stringify({ error: "Nazwa jest wymagana" }), {
      status: 400,
      headers: corsHeaders,
    });
  }
...
async function getTasks(params, env, corsHeaders) {
  ...
  query +=
    ' ORDER BY t.sort_order ASC, CASE t.priority WHEN "high" THEN 1 WHEN "normal" THEN 2 WHEN "low" THEN 3 END, t.scheduled_time ASC';
...
async function updateTaskStatus(id, request, env, corsHeaders) {
  ...
  await env.DB.prepare(
    ...
    .bind(id, userId, `Status zmieniony na: ${status}`)
    .run();
  ...
  for (const admin of admins.results) {
    await env.DB.prepare(
      ...
      .bind(admin.id, `Zadanie "${task.description}" zostało ${statusText}`, id)
      .run();
  }
...
async function createTaskLog(taskId, request, env, corsHeaders) {
  ...
  if (logType === "delay" || logType === "problem") {
    ...
    const title =
      logType === "delay" ? "Przestój zgłoszony" : "Problem zgłoszony";
